...
async function seed() {
  ...
  await prisma.user.create({
    data: { username: "trader", balance: 10000 },
  });
  ...
  for (const m of markets) {
    ...
    const market = await prisma.market.create({
      data: {
        ...
        category: m.category,
        yesPrice: m.yesPrice,
        noPrice: +(1 - m.yesPrice).toFixed(2),
    ...
    const historyPoints: { marketId: string; yesPrice: number; noPrice: number; volume: number; timestamp: Date }[] = [];
